// PA Copilot local rule engine — static reference data. No PHI stored here.

const RULE_ENGINE_VERSION = "2026.06.1";
const REFERENCE_LAST_UPDATED = "2026-06-01";

const PAYER_PORTAL_MATRIX = [
...
function matchPortal(payerName) {
  const name = (payerName || "").toLowerCase();
  for (const [hint, portal] of Object.entries(PAYER_HINTS)) {
    if (name.includes(hint)) {
      const match = PAYER_PORTAL_MATRIX.find((p) => p.portal === portal);
      if (match) return { ...match, auto_matched: true };
    }
  }
  return { ...PAYER_PORTAL_MATRIX[0], auto_matched: false };
}

const CODE_CROSSWALK = {
  ...
  "M79.7": [{ code: "20610", desc: "Major joint injection", ndc: null, confidence: 0.71, policy: "LCD — requires documented conservative trial" }],
  "E11.9": [{ code: "J1815", desc: "Insulin injection (various)", ndc: "varies", confidence: 0.9, policy: "Formulary tier + step-therapy edit" }],
  "C50.911": [{ code: "77301", desc: "IMRT planning", ndc: null, confidence: 0.88, policy: "NCD 50.1 / payer radiation-oncology medical policy" }],
  ...
  "J45.50": [{ code: "J2786", desc: "Reslizumab injection", ndc: "59310-105-01", confidence: 0.8, policy: "Eosinophil count threshold + ICS/LABA trial" }],
};

const CONFIDENCE_AMBIGUOUS_THRESHOLD = 0.75;

function crosswalkForIcds(icdCodes) {
  const rows = [];
  for (const icd of icdCodes || []) {
    const key = (icd || "").trim().toUpperCase();
    const candidates = CODE_CROSSWALK[key];
    if (candidates) {
      ...
      rows.push({
        icd10: key, code: null, description: null, ndc: null,
        confidence: 0.0, policy: "No preset cross-walk — manual code entry required", ambiguous: true,
      });
    }
  }
  return rows;
...
const PLACE_OF_SERVICE = ["Inpatient", "Outpatient", "Home", "Office", "Ambulatory Surgical Center", "Telehealth"];

function getPresets() {
  return { quantity_presets: QUANTITY_PRESETS, modifier_presets: MODIFIER_PRESETS, place_of_service: PLACE_OF_SERVICE };
}

function policyContextFor(codes) {
  const ctx = [];
  for (const rows of Object.values(CODE_CROSSWALK)) {
    for (const c of rows) {
      if ((codes || []).includes(c.code)) {
        ctx.push({ code: c.code, source: "Static reference (rule engine)", note: c.policy });
      }
    }
  }
  return ctx;
...
module.exports = {
  PAYER_PORTAL_MATRIX, matchPortal, crosswalkForIcds, getPresets, policyContextFor, referenceMeta,
};
